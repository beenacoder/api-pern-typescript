...
import { handleInputErrors } from "./middlewares/errors.js";
...
router.post('/',
    //Validación
    body('name')
        .notEmpty().withMessage('Debes ingresar un nombre para producto'),
    body('price')
        .isNumeric().withMessage("Debe ser un número")
        .notEmpty().withMessage('Debes ingresar un precio para el producto')
        .custom(val => val > 0).withMessage('El valor debe ser mayor que 0'),
    handleInputErrors,
    createProduct);
